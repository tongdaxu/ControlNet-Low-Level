...
class DiffClampFunction(torch.autograd.Function):
    @staticmethod
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    semb, temb, hist = self.controlnet(
                        latents_hat,
                        t,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    norms = []
                    for i in range(16):
                        normi = torch.linalg.norm(f(pred_x0) - y)
                        norms.append(normi[None])
                    norms = torch.cat(norms, dim=0)
                    weights = F.softmax(-norms, dim=0)
                    norm = torch.sum(norms * weights)
...
class StableDiffusionControlNetInverse(StableDiffusionControlNetPipeline):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    # compute the previous noisy sample x_t -> x_t-1                    
                    latents_next, pred_z0, latents_next_mean, sigma_up = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)
    ...
    def dsg_step(self, f, y, latents, prompt_embeds, timestep_cond, t, cond_scale, image, generator, added_cond_kwargs, extra_step_kwargs):
        ...
        return norm, norm_grad, latents_next, pred_z0, latents_next_mean, sigma_up
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # compute the previous noisy sample x_t -> x_t-1                    
                latents_next, _, latents_next_mean, sigma_up = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)

                for j in range(3):
                    if i != len(timesteps) - 1:
                        norm, norm_grad, _, _, _, _ = self.dsg_step(f, y, latents_next, prompt_embeds, timestep_cond, timesteps[i + 1], cond_scale, image, generator, added_cond_kwargs, extra_step_kwargs)
                        self.scheduler._step_index = self.scheduler._step_index - 1
                    else:
                        self.scheduler._step_index = self.scheduler._step_index - 1
                        norm, norm_grad, _, _, _, _ = self.dsg_step(f, y, latents_next, prompt_embeds, timestep_cond, t, cond_scale, image, generator, added_cond_kwargs, extra_step_kwargs)
                    
                    progress_bar.set_postfix({'distance': norm.item()}, refresh=False)
                    # project on to sphere
                    latents_star = latents_next - norm_grad * scale
                    latent_diff_star = latents_star - latents_next_mean
                    latent_diff_star_rescale = (latent_diff_star /  torch.linalg.norm(latent_diff_star)) * sigma_up * 128.0 # 128**2 = 64 * 64 * 4

                    latents_next = latents_next_mean + latent_diff_star_rescale
...
class StableDiffusionInverse(StableDiffusionPipeline):
    ...
    def rsp_lo(self, y, z_init, f, generator, eps=1e-3, max_iters=200, lr=None):
        ...
        with torch.enable_grad():
            ...
            y = y.detach() # Need to detach for weird PyTorch reasons
            ...
            for itr in range(max_iters):
                optimizer.zero_grad()
                x_init = self.vae.decode(z_init / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
                output = loss(y, f(x_init))          
    ...
    def rsp_resample(self, pseudo_x0, x_t, a_t, sigma):
        """
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i % 50 == 0 and i != 0 and i <= num_inference_steps // 5:
                    pred_z0_rsp, _ = self.rsp_lo(y.detach().clone(), pred_z0.detach().clone(), f, generator)
                    a_t = self.scheduler.alphas_cumprod[i]
                    a_prev = self.scheduler.alphas_cumprod[i - 1]
                    sigma = 40 * (1-a_prev)/(1 - a_t) * (1 - a_t / a_prev)
                    latents = self.rsp_resample(pred_z0_rsp, latents, a_prev, sigma)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    pred_x0 = self.vae.decode(pred_z0 / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
                    if type(f) == LayoutOperator:
                        logits = f(pred_x0)
                        norm = F.cross_entropy(logits, y[:,0].to(torch.long))
                    else:
                        norm = torch.linalg.norm(f(pred_x0) - y)
                    norm_grad = torch.autograd.grad(outputs=norm, inputs=latents)[0]
                    progress_bar.set_postfix({'distance': norm.item()}, refresh=False)
            ...
            for i, t in enumerate(timesteps):
                ...
                for j in range(K):
                    ...
                    if j != (K-1):
                        ...
                        latents = torch.sqrt(1 - beta_t) * latents + torch.sqrt(beta_t) * eps
                        self.scheduler._step_index = self.scheduler._step_index - 1
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    scheduler_out = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)
                    latents_next, pred_z0 = scheduler_out.prev_sample, scheduler_out.pred_original_sample
                    pred_z0 = pred_z0 + torch.randn_like(pred_z0) * 0.2
                    pred_x0 = self.vae.decode(pred_z0 / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
                    if type(f) == LayoutOperator:
                        logits = f(pred_x0)
                        norm = F.cross_entropy(logits, y[:,0].to(torch.long))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                for j in tqdm(range(1000)):
                    ...
                    offset = norm_grad * scale
                    noise_pred_dps = noise_pred + offset / coeff
                    score_uncond = noise_pred * torch.sqrt(beta_prod_t)
                    score_dps = noise_pred_dps * torch.sqrt(beta_prod_t)
                    if j == 0:
                        score_uncond_sum = score_uncond
                        score_dps_sum = score_dps
                    else:
                        score_uncond_sum = torch.cat([score_uncond_sum, score_uncond])
                        score_dps_sum = torch.cat([score_dps_sum, score_dps])
                return score_uncond_sum, score_dps_sum
    ...
    def run_dpscm(
        ...
        **kwargs,
        ...
        cms.set_timesteps(lcm_origin_steps, lcm_origin_steps, device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    # compute the previous noisy sample x_t -> x_t-1
                    _, pred_z0 = cms.step(cm_pred, itcm, tcm, latents, return_dict=False)
                    ...
                    # compute the previous noisy sample x_t -> x_t-1                    
                    scheduler_out = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)
                    latents_next, _ = scheduler_out.prev_sample, scheduler_out.pred_original_sample

                    pred_x0 = self.vae.decode(pred_z0 / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
                    pred_x0 = DiffClampFunction.apply(pred_x0)
                    # add a little noise to avoid overfitting
                    pred_x0 = pred_x0 + torch.randn_like(pred_x0) * 0.2
    ...
    def run_lcm(
        ...
        **kwargs,
        ...
        # 0. Default height and width to unet
        height = height or cm.config.sample_size * self.vae_scale_factor
        width = width or cm.config.sample_size * self.vae_scale_factor
        ...
        # 3. Encode input prompt
        prompt_embeds = self._encode_prompt_lcm(
            prompt,
            device,
            num_images_per_prompt,
            prompt_embeds=prompt_embeds,
        ...
        # 4. Prepare timesteps
        cms.set_timesteps(num_inference_steps, lcm_origin_steps)
        timesteps = cms.timesteps
        
        # 5. Prepare latent variable
        num_channels_latents = cm.config.in_channels
        latents = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                model_pred = cm(
                    ...
                    return_dict=False)[0]
                # compute the previous noisy sample x_t -> x_t-1
                latents, denoised = cms.step(model_pred, i, t, latents, return_dict=False)
                print(i, t, torch.min(denoised), torch.max(denoised))
    ...
    def dsg_step(self, f, y, latents, prompt_embeds, timestep_cond, t, generator, added_cond_kwargs, extra_step_kwargs):
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # takes a step as initial step
                latents_next, pred_z0, latents_next_mean, sigma_up = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)

                for j in range(3):
                    if i != len(timesteps) - 1:
                        norm, norm_grad, _, _, _, _ = self.dsg_step(f, y, latents_next, prompt_embeds, timestep_cond, timesteps[i + 1], generator, added_cond_kwargs, extra_step_kwargs)
                        self.scheduler._step_index = self.scheduler._step_index - 1
                    else:
                        self.scheduler._step_index = self.scheduler._step_index - 1
                        norm, norm_grad, _, _, _, _ = self.dsg_step(f, y, latents_next, prompt_embeds, timestep_cond, t, generator, added_cond_kwargs, extra_step_kwargs)
                    
                    # as if it is not taken
    ...
    def run_pt(
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                prompt_opt = torch.optim.SGD([prompt_embeds], lr=pt_lr)
                if self.interrupt:
                    continue
                if i <= 100:
                    PTSTEP = 4
                ...
                for j in range(PTSTEP):
                    with torch.enable_grad():
                        ...
                        # compute the previous noisy sample x_t -> x_t-1
                        scheduler_out = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)
                        if j != PTSTEP - 1 and hasattr(self.scheduler, '_step_index'):
                            self.scheduler._step_index = self.scheduler._step_index - 1
            ...
            for i, t in enumerate(timesteps):
                ...
                for j in range(5):
                    with torch.enable_grad():
                        ...
                        noise_pred, noise_pred_hat = noise_pred.chunk(2)

                        scheduler_out = self.scheduler.step(noise_pred, t, latents, return_dict=True, **extra_step_kwargs)
                        pred_z0 = scheduler_out.pred_original_sample
                        # compute the previous noisy sample x_t -> x_t-1

                        alpha_prod_t = self.scheduler.alphas_cumprod[t]
                        coeff = - torch.sqrt(1 - alpha_prod_t)

                        score = noise_pred * coeff
                        score_hat = noise_pred_hat * coeff
                        ...
                        dist = torch.linalg.norm(f(pred_x0) - y)
                        stsl_eta = 10
                        norm = scale * dist + stsl_eta * diff
                        norm_grad = torch.autograd.grad(outputs=norm, inputs=latents)[0]

                        progress_bar.set_postfix({'distance': dist.item()}, refresh=False)

                    latents = latents.detach() - norm_grad
