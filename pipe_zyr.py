...
class StableDiffusionInverse(StableDiffusionPipeline):
    ...
    def run_multi_dps(
        ...
        **kwargs,
        ...
        tt = 40
        timestart = timesteps[tt:tt+1]
        timesteps = timesteps[tt:]
        latents = self.scheduler.add_noise(latents, latent_noise, timestart)
        print(timesteps)
        assert(0)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                denoised_image = self.image_processor.postprocess(denoised, output_type=output_type)[0]
                denoised_image.save("./zyr_folder/DPS_test_x_pred_zyr_{}.png".format(i))
        ...
        for latent_optimize_time in range(500):
            latent_copy = latents.detach()
            with torch.enable_grad():
                latent_copy.requires_grad = True
                latent_image = (latent_copy / self.vae.config.scaling_factor) + self.vae.config.shift_factor
                image = self.vae.decode(latent_image, return_dict=False)[0]    
                norm = torch.linalg.norm(f(image) - y)
                norm_grad = torch.autograd.grad(outputs=norm, inputs=latent_copy)[0]
                latent_copy = latent_copy - 1.0 * norm_grad
            print("distance: {:.4f}".format(norm.item()))
            latents = latent_copy
            
            latents_proxy = (latents / self.vae.config.scaling_factor) + self.vae.config.shift_factor

            image = self.vae.decode(latents_proxy, return_dict=False)[0]
            
            image = self.image_processor.postprocess(image, output_type=output_type)[0]
            image.save("./zyr_folder/test_x_pred_zyr_decoder_{}.png".format(latent_optimize_time))
